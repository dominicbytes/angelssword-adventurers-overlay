...
The optional **Viseme Lip Sync** plugin analyzes the already-selected microphone locally and selects among four mouth shapes: closed, open, wide, and round. It supports phase-aligned full-video swaps plus a v2 sprite mode that keeps one mouth-neutral body loop running and composites only the mouth. The Control Panel includes continuous openness, local voice calibration, and no-microphone test controls.
